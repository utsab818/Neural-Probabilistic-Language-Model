...
V, m, B, k = 30, 6, 5, 4
model = Word2Vec(vocab_size=V, embed_dim=m, seed=1)

center_ids = rng.integers(0, V, size=B)
context_ids = rng.integers(0, V, size=B)
negative_ids = rng.integers(0, V, size=(B, k))

p_pos, p_neg, cache = model.forward(center_ids, context_ids, negative_ids)
grads = model.backward(cache, center_ids, context_ids, negative_ids)


def loss_now():
    pp, pn, _ = model.forward(center_ids, context_ids, negative_ids)
